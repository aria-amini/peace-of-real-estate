...
const USE_MEMORY_IN_DEV = import.meta.env.DEV

let devDraft: IntakeDraft | null = null
...
export function saveStoredIntakeDraftForRole(
	...
	updates: Partial<
		Omit<
			IntakeDraft,
			'role' | 'flowKind' | 'answers' | 'agentProfile' | 'updatedAt'
		>
	> & {
...
export function getNextPathForConsumerFlow(
	...
	switch (stage) {
		...
		default:
			...
			if (draft.zipCode && draft.propertyType?.length) {
				return `${base}/situation`
			}
			return `${base}/intro`
	}
